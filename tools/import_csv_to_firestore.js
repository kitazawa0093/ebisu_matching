/**
 * CSV（category,question,answer,tags,is_public）を Firestore manual_items に一括登録する
 *
 * 使い方：
...
const CSV_PATH = process.argv[2] ? path.resolve(process.argv[2]) : DEFAULT_CSV_PATH;
...
// is_public は TRUE/FALSE, true/false, 1/0 など対応
function normalizeBool(v) {
  const s = String(v || "").trim().toLowerCase();
  return s === "true" || s === "1" || s === "yes" || s === "y" || s === "ok";
}

// questionをdocIdに使う（上書きできるように）
...
async function main() {
  ...
  const batchSize = 450; // Firestore batch上限 500 に余裕
  let imported = 0;
  let skipped = 0;
